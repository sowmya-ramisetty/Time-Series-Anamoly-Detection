...
class SAND_model():
    ...
    def fit(self, X, y=None, online=True, alpha=None, init_length=None, batch_size=None, overlaping_rate=10,
            verbose=True):
        ...
        if online:
            if (alpha is None) or (init_length is None) or (batch_size is None):
                logger.warning("You must specify a value for alpha, init_length, and batch_size")
                return None
            ...
            if verbose:
                logger.info("[STOP]: score length {}".format(len(self.decision_scores_)))
