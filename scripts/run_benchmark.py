...
def build_data_config(args: argparse.Namespace, config_data: Dict) -> Dict:
    ...
    if args.data_name_list is not None:
        data_config["data_name_list"] = [os.path.basename(s) for s in args.data_name_list]
...
def build_model_config(args: argparse.Namespace, config_data: Dict) -> Dict:
    ...
    for adapter, model_name, model_hyper_params in zip(
        ...
        if model_hyper_params is not None and os.path.exists(model_hyper_params):
            with open(model_hyper_params, "r") as f:
                hyper_params = json.load(f)
        elif model_hyper_params is not None:
            hyper_params = json.loads(model_hyper_params)
        else:
            hyper_params = {}
